...
export const login = (name, email, password) => async (dispatch) => {
  ...
  console.log(body);
  try {
    const res = await axios.post("http://localhost:4000/api/auth/signin-user", body, config);
...
export const signup = (name, email, password) => async (dispatch) => {
  ...
  try {
    console.log(body);
    const res = await axios.post("http://localhost:4000/api/auth/register-user", body, config);
    dispatch({
      type: SIGNUP_SUCESS,
      payload: res.data,
    });
